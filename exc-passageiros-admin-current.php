                ...
                <div class="wpp-link-container">
                  ...
                  <div class="email-wpp-link-container">
                    ...
                    <dialog id="email-wpp-<?= $variacao['variation_id'] ?>">
                      ...
                      <span class="confirma-envio-email" data-link="<?= $link_grupo_wpp; ?>" onclick="enviaEmailLinkWpp(this)">Clique aqui para continuar >></span>

<!-- 
                      <span class="teste-envia-email" data-link="<?php //echo $link_grupo_wpp; ?>"  data-emails='["rterragd@hotmail.com", "rterraredator@gmail.com", "rterra@rterra.xyz"]' onclick="enviaEmailLinkWpp(this)">Clique aqui para continuar >></span> -->
    ...
    /* CLIQUE NO SWITCH DE LINHAS - ajax request */
        ...
        if(button.innerText === 'Clique aqui para continuar >>'){
          ...
          jQuery(function($) {
            $.ajax({
              ...
              success: async function(response) {
                if(response.data.every(_value => _value === true)){
                  button.innerText = response.data.length + " e-mails enviados com sucesso.";
                  button.style.cursor = 'text';
                }else{
                  const emailsSucesso = response.data.flatMap(_value => _value === true ? _value : []);
                  const emailsErro = response.data.flatMap(_value => _value !== true ? _value : []);
                  button.innerText = emailsSucesso.length + "e-mails enviados com sucesso e " + emailsErro.length + "envios falharam: " + emailsErro.join(', ')
                }
              },
              error: function(error) {
                console.log('response error:  ' + error);
      ...
      document.querySelectorAll('.meta_encerrar_vendas_var_input').forEach(inp => inp.addEventListener('change', ({target}) => {
          ...
          $.ajax({
            ...
            success: async function(response) {
              ...
              const _user_ids = response.data[0].flatMap((_r) =>{
                if(_r.user_id != '0'){
                  if(_r.status === 'normal') return +_r.user_id;
                }
                return [];
              });
              ...
              jQuery(function($) {
                $.ajax({
                  ...
                  success: async function(_resp){
                    ...
                    dialogElement.querySelector('.confirma-envio-email').dataset.emails = JSON.stringify(_resp.data);
                    dialogElement.querySelector('.confirma-envio-email').dataset.excName = exc_name;
                    dialogElement.querySelector('.confirma-envio-email').dataset.excData = exc_data;
                  },
      ...
      if(containersEmailLinkWpp){
        ...
        function toggleContainer(__e){
          ...
          wppInput.addEventListener('change', ({target}) => {
