...
<body <?php body_class(); ?>>
  ...
  ?>
  <header class="hero-container <?= is_front_page()? '' : 'inner-header' ?>" id="aer_header">
    <?php
      $todas_campanhas = $wpdb -> get_results($wpdb -> prepare("SELECT `id`,`nome_campanha`,`valido_de`, `valido_ate`, `status` from `aer_camp_premios`"));

      $campanhas_ativas = array_values(array_filter($todas_campanhas, function($_camp){
    ...
    <div class="topbar d-flex justify-between py-3">
      <div class="logo">
        <a href="<?= get_home_url(); ?>">
          <img src="<?= esc_url( get_theme_mod( 'aer_logo') ); ?>" alt="<?= esc_attr(get_post_meta(attachment_url_to_postid(get_theme_mod('aer_logo')), '_wp_attachment_image_alt', true)); ?>">
        </a>
      </div>
      <div class="navbar-flex-wrapper">
        ...
        <div class="user">
          <?= !is_user_logged_in() ? '<a href="' . wc_get_page_permalink( 'myaccount' ) . '">' : ''; ?>
          <div class="topbar-user-wrapper">
              <div class="<?= is_user_logged_in() ? 'usuario-logado': ''?> d-flex">
              
                <div class="saudacao d-flex align-items-center gap-2"><?= aer_icons("user", 16, 16)?><span><?= is_user_logged_in() ? $user->display_name : 'Olá, visitante'; ?></span></div>
                <?php 
            if(is_user_logged_in()){ 
              ?>
                <div class="user-menu-container user-menu-btn" onclick="openModalBox('user-menu-modal')">
                    ...
                    <?php
                      wp_nav_menu( ['menu' => 'Usuário header', 'container_id' => 'user-menu-modal', 'container_class' => 'd-none'] );
                    ?>
      ...
      <div class="topbar-social">
        <a href="https://www.facebook.com/aerotourcampinas/" target="_blank" aria-label="Ícone do Facebook"><?= aer_icons('facebook', 26, 26) ?></a>   
        <a href="https://www.instagram.com/aerotour_excursoes/" target="_blank" aria-label="Ícone do Instagram"><?= aer_icons('instagram',26, 26) ?></a> 
      </div>
    </div>
