    ...
    <form class="woocommerce-cart-form col-lg-8" action="<?php echo esc_url(
      wc_get_cart_url()
       ...
       $nome_embarque = $wpdb->get_var(
         $wpdb->prepare('SELECT nome FROM aer_embarques WHERE id = %d', $emb_id)
       );
       ...
       if (
         ...
         $product_permalink = apply_filters(
           ...
           $product_permalink
         ); ?>"><h3 class="bg-title">Excursão <?php echo esc_html(
  preg_replace('/ - \d{2}\/\d{2}\/\d{4}$/', '', $product_name)
); ?></h3></a>
									...
									<div class="d-flex justify-content-between">
										...
										<?php if ($nome_embarque): ?>
											<p><strong>Embarque:</strong> <?php echo esc_html($nome_embarque); ?></p>
										<?php endif; ?>

											<p><strong>Valor:</strong> <?php
           if (
             $cart_item['data']->get_meta('preco_original') &&
             $cart_item['data']->get_meta('desconto_antecipado_rev') !== false
           ) {
           ...
           ?></p>
									<?php if (
           isset($cart_item['desconto_antecipado']) &&
           $cart_item['data']->get_meta('desconto_antecipado_rev') === true
         ) {
           echo '<p class="desconto-antecipado-alert">Desconto de 5% válido até ' .
             esc_html(
               data_to_dmy($cart_item['data']->get_meta('data_limite_desconto'))
             ) .
             '</p>';
         } ?>
								</div>
            ...
            $passageiros
          ) ?>"class="toggle-passengers secondary-btn toggle-active">Ver passageiros (<?= count(
  $passageiros
) ?>)</button>
             ...
             ?>
													<div>
													<p><strong>Nascimento:</strong> <?php echo esc_html(
							...
							<?php if (wc_coupons_enabled()) { ?>
								<div class="coupon">
									...
									<div id="coupon_inputs">
										<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e(
            'Coupon code',
            'woocommerce'
          ); ?>" /> <button data-btn-reactive type="submit" class="button<?php echo esc_attr(
  wc_wp_theme_get_element_class_name('button')
    ? ' ' . wc_wp_theme_get_element_class_name('button')
    : ''
); ?> btn cart-btn-style" name="apply_coupon" value="<?php esc_attr_e(
   'Apply coupon',
   'woocommerce'
