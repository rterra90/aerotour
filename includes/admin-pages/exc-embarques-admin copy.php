...
// FUNÇÃO SALVAEMBARQUES ATUALIZADA
function salvaEmbarques() {
    const dataPorVariacao = {};
    const variacoesEls = document.querySelectorAll('.variacoes-data .variacao-item');
console.log('variacoesEls:', variacoesEls);
    variacoesEls.forEach(varEl => {
        const varId = varEl.dataset.varid;
        dataPorVariacao[varId] = [];

        const embarquesAtivos = document.querySelectorAll('.embarques-disponiveis-lista .item-embarque');

        embarquesAtivos.forEach(embEl => {
            const embId = parseInt(embEl.dataset.empid);
            const taxaInput = embEl.querySelector('.taxa-embarque');
            const taxaVal = taxaInput ? parseFloat(taxaInput.value) || 0 : 0;

            const diasDisponibilidade = embEl.querySelectorAll('.dias-disponibilidade-lista .dia-item');

            diasDisponibilidade.forEach(diaItem => {
                const disponibilidades = diaItem.querySelectorAll('.disponibilidade');

                disponibilidades.forEach(dispEl => {
                    const dispVarId = dispEl.dataset.varid;
                    const horaStr = dispEl.dataset.horario;
                    
                    // Lê o valor do novo input oculto
                    const statusVal = dispEl.querySelector('.disp-status-input').value;

                    if (dispVarId === varId) {
                        let embConfig = dataPorVariacao[varId].find(e => e.embarque_id === embId);
                        if (!embConfig) {
                            embConfig = {
                                embarque_id: embId,
                                taxa: taxaVal,
                        ...
                        // Salva as duas informações para não quebrar o site
                        embConfig.horarios.push({
                            horario: horaStr,
                            status: statusVal, // NOVO: 'disponivel', 'esgotado', ou 'indisponivel'
                            disponivel: (statusVal === 'disponivel') // FALLBACK: true ou false
                        });
                    }
                });
            });
    ...
    const hiddenInput = document.querySelector('input#meta_embarques_data');
    if (hiddenInput) {
        console.log('Salvando dados de embarques:', dataPorVariacao); // Log para depuração
        hiddenInput.value = JSON.stringify(dataPorVariacao);
